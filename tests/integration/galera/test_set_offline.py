...
def test__galera_server_set_offline_reader_if_backends_are_registered_with_empty_mysql_servers(percona_xtradb_cluster_three_node,
                                                                                               proxysql_instance,
                                                                                               tmpdir):
...
def test__galera_server_set_offline_writer_if_backends_are_registered(percona_xtradb_cluster_three_node,
    ...
    assert result.exit_code == 0
    result = runner.invoke(main,
                           ['--config', config_file, 'galera', 'server', 'set_offline', percona_xtradb_cluster_three_node[0]['ip'], '3306']
                           )
    assert result.exit_code == 0
...
def test__galera_server_set_offline_writer_if_backends_are_registered_with_empty_mysql_servers(percona_xtradb_cluster_three_node,
                                                                                               proxysql_instance,
                                                                                               tmpdir):
